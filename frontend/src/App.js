 ...
 */
  ...
  async function sendLogin(email, password) {
    ...
    try {
      const res = await fetch(`https://tigertix-user-auth.up.railway.app:4000/api/auth/login`,
        {
          method: "POST",
  ...
  async function sendSignUp(name, email, password) {
    ...
    try {
      const res = await fetch(`https://tigertix-user-auth.up.railway.app:4000/api/auth/register`, requestOptions);
      console.log(res);
      if (!res.ok) throw new Error("Message failed to send");
